...
const option=document.querySelector('.menu2');
const attempted_question=document.getElementById('attempted_question');
const no_of_question=document.getElementById('no_of_question');
const next=document.getElementById('next_page');



let correctAns=" ",correctScore =0,askedCount=0 ,totalQuestion =10;
...
document.addEventListener('DOMContentLoaded',()=>{
    getQuestion();
    eventListeners();
    no_of_question.textContent=totalQuestion;
    attempted_question.textContent=askedCount;
...
function  selectOption(){
    option.querySelectorAll('a').forEach((option)=>{
        option.addEventListener('click',()=>{
           if(option.querySelector('.selected')){
            const activOption=option.querySelector('.selected');
            activOption.classList.remove('selected');
           }
